...
def _interpret_filters(filters: dict[str, Any]) -> dict[str, Any]:
    ...
    if pos:
        pos_text = ", ".join(str(p) for p in pos) if isinstance(pos, list) else str(pos)
        if pos_text.strip():
            meaningful_thresholds.append(
...
def _score_case(packet: dict[str, Any]) -> tuple[int, str]:
    ...
    top_pos_tiers = sum(
        ...
        and (
            (info.get("percentile_top") is not None and float(info.get("percentile_top")) >= 90)
            or "top" in str(info.get("tier") or "").lower()
        )
    )
...
def compose_hof_statistical_case(packet: dict[str, Any]) -> dict[str, Any]:
    ...
    for stat in strengths[:4]:
        info = ranks.get(stat)
        if isinstance(info, dict) and (line := _fmt_rank(info)):
    ...
    return {
        ...
        "supporting_points": strongest[:4] + ([weakest[0]] if weakest else []),
