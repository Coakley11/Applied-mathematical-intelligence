...
def _import_storage() -> Any:
    """
    Resolve suite storage backend.

    Command Center ships ``suite_storage`` (cloud + SQLite shim). Standalone suite
    apps (AMI, Investment) typically only have ``suite_storage_supabase``.
    """
    try:
