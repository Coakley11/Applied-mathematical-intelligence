...
def _apply_applied_intelligence(st: Any, page: str) -> None:
    ...
    if question:
        st.session_state["_suite_ai_question"] = question
        st.session_state["ps_library_problem"] = question
    source_app = _qp_get(st, "suite_ai_source_app")
    if source_app:
        st.session_state["_suite_ai_source_app"] = source_app
    source_page = _qp_get(st, "suite_ai_source_page")
    if source_page:
        st.session_state["_suite_ai_source_page"] = source_page
    area = _qp_get(st, "suite_ai_area")
    if area:
        st.session_state["ps_area_id"] = area
        st.session_state["_suite_ai_area"] = area
    ctx = _qp_get(st, "suite_ai_context")
    if ctx:
        st.session_state["_suite_ai_context"] = ctx
    if question:
